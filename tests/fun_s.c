...
// calcular la distancia euclidea entre dos vectores
    float distancia = elem1 - elem2;
    return sqrt(pow(distancia,2));
}
...
double distancia;
       ...
       for (int i = 0; i < nelem; i++)
       {
              ...
              for (int j = 0; j < NGRUPOS; j++)
              {
                     ...
                     distancia = gendist(elem[i], cent[i]);
                     if (min > distancia)
                     {
                            auxiliar = j;
                            min = distancia;
              ...
              for (int j = 0; j < listag[i].nelemg; j++)
              {
                     ...
                     for (int k = 0; k < listag[i].nelemg; k++)
                     {
                            ...
                            if (j != k)
                            {
                                   ...
                                   suma = suma + gendist(listag[i].elemg[j], listag[i].elemg[k]);
                                   contadordistancias++;
                            }
                     }
              }

              densidad[i] = suma / contadordistancias;
       }
}
       ...
       for (int i = 0; i < NGRUPOS; i++)
       {
              ...
              for (int k = 0; k < TENF; k++)
              {
                     ...
                     for (int j = 0; j < listag[i].nelemg; j++)
                     {
                            ...
                            suma = suma + enf[listag[i].elemg[j]][k];
                     }

                     mediaenfermedad = suma / listag[i].nelemg;

                     if (mediaenfermedad > prob_enf[k].max){
                            prob_enf[k].max = mediaenfermedad;
                            prob_enf[k].gmax = i;
                     }
                        if (mediaenfermedad <= prob_enf[k].min){
                            prob_enf[k].min = mediaenfermedad;
                            prob_enf[k].gmin = i;
                     }
              }
       }
